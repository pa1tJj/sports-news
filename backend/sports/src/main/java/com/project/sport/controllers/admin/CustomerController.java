...
public class CustomerController {
	...
	public ResponseEntity<DataResponses> getUsersByRoleCustomer(CustomerSearchRequest request) {
		try {
			dataResponses.setList(userService.findUsersByRoleCustomer(request, PageRequest.of(request.getPage() - 1, request.getPageSize())));
			dataResponses.setTotalPage(userService.totalPageCustomer());
