...
public class NewsConverter {
	...
	private void mapCommon(NewsEntity newsEntity, Set<CategoryEntity> categoryEntities, Set<TagsEntity> tagsEntities) {
	...
	public NewsEntity toNewsEntityCreate(NewsSaveRequest newsSaveRequest, MultipartFile images,
			Set<CategoryEntity> categoryEntities, Set<TagsEntity> tagsEntities) {
		...
		mapCommon(newsEntity, categoryEntities, tagsEntities);
		return newsEntity;
	}

	public NewsEntity toNewsEntityUpdate(NewsSaveRequest newsSaveRequest, MultipartFile images,
			NewsEntity newsOld, Set<CategoryEntity> categoryEntities, Set<TagsEntity> tagsEntities) {
		NewsEntity newsEntity = newsOld;
		mapper.map(newsSaveRequest, newsOld);
		if (images != null) {
			newsEntity.setThumbnail(fileStorageService.saveImage(images));
		}
		mapCommon(newsEntity, categoryEntities, tagsEntities);
	...
	public NewsDetailResponse toNewsDetail(NewsEntity newsEntity) {
		...
		newsDetailResponse.setCategories(newsEntity.getCategories().stream()
				.collect(Collectors.toMap(CategoryEntity::getId, CategoryEntity::getName)));
		newsDetailResponse.setTags(
				newsEntity.getTags().stream().collect(Collectors.toMap(TagsEntity::getId, TagsEntity::getName)));
		newsDetailResponse.setThumbnailPreview(newsEntity.getThumbnail());
		return newsDetailResponse;
	}

	
	public TopPostDTO toTopPostDTO(NewsEntity newsEntity) {
		return mapper.map(newsEntity, TopPostDTO.class);
	}
}
